...
const validationMessages = {
  ...
  invalidZipcode: "Zip code should be 3-20 characters",
};
...
const validateForm = (form) => (value) => {
  ...
  if (form === "newsletter") {
    ...
    // Validate address fields (optional but if provided must meet criteria)
    if (value.address1 && value.address1.length < 5) {
      errors.address1 = validationMessages.short("Address line 1");
    }
    if (value.address2 && value.address2.length < 2) {
      errors.address2 = validationMessages.short("Address line 2");
    }
    if (value.zipcode && (value.zipcode.length < 3 || value.zipcode.length > 20)) {
      errors.zipcode = validationMessages.invalidZipcode;
    }
